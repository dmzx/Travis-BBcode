...
class update_table extends \phpbb\db\migration\migration
{
	...
	public function install_bbcode_for_travisbbcode()
	{
		...
		$bbcode_array = array(
		 ...
		 'bbcode_tpl'		=> '<div><a href="https://travis-ci.org/{IDENTIFIER1}/{IDENTIFIER2}" title="{IDENTIFIER1}/{IDENTIFIER2}"><img src="https://travis-ci.org/{IDENTIFIER1}/{IDENTIFIER2}.svg?branch=master" alt="" style="margin-bottom: -5px;" /></a></div>',
	...
	{
		$sql = 'DELETE FROM ' . BBCODES_TABLE . ' WHERE bbcode_tag = \'travis\'';
		$this->db->sql_query($sql);
	}
}
